...
class App:
    def __init__(self, window, window_title, video_source=stream):
        self.window = window
        ...
        self.canvas = tk.Canvas(window, width = self.vid.width // 2, height = self.vid.height // 2)
        self.canvas.pack()
...
class VideoCapture:
    def __init__(self, video_source=stream):
        ...
        self.out = cv2.VideoWriter(args.name[0]+'.'+args.type[0], self.fourcc, 30, res)
